...
startBtn.addEventListener('click', ()=>{
  ...
  music.play();
  startAnimation(); // Mulai otomatis animasi foto → teks
});
...
// Stars
for(let i=0;i<50;i++){
  particles.push({x:Math.random()*400, y:Math.random()*400, r:Math.random()*2+1, dx:(Math.random()-0.5)*2, dy:(Math.random()-0.5)*2});
}

// Animasi Foto Berputar
function startAnimation(){
  function animate(){
    ...
    step++;
    if(step < 200){ // Setelah beberapa detik → pecah foto jadi teks
